...
const fetchData = async (event) => { 
    ...
    try {
        const response = await store.getData({
            url     :  `api/<?php echo e($moduleName); ?>?s=${textSearch.value}`  
        })  
...
const add = async (event) => { 
    loading.value = true ;
    const response = await store.getData({
        url     :  `api/<?php echo e($moduleName); ?>/create`  
    })  
    if (response.data.status === 1) {   
...
const edit = async (item) => { 
    if(is_edit.value == 1) { 
        loading.value = true ;
        const response = await store.getData({
            url     :  `api/<?php echo e($moduleName); ?>/edit?id=${item.id}`  
        })  
        if (response.data.status === 1) {   
            posts.value = response.data.data 
            modal.value = true 
...
const doDownload = async (event) => {
    ...
    try {
        const response = await store.getData({
            url     :  `api/<?php echo e($moduleName); ?>/download?dw=${download.value.startRows}&e=${download.value.endRows}&t=${download.value.type}&s=${textSearch.value}`  
        })  
        proxy.$export(  response.data ,  '<?php echo e($moduleName); ?>.xls');
        proxy.$messageSuccess(  proxy ,  'Success' ,response.data.message )        
...
const doPrint = async (event) => {  
    proxy.$printArea( 'printedArea','<?php echo e($moduleTitle); ?>')    
}
const remove  = async (event) => {
    ...
    if (result.isConfirmed) {
        const response = await store.deleteData({
            url: "api/<?php echo e($moduleName); ?>/" + selected.value ,
            headers: {
                "Content-Type": "application/json",
            },
        responseType: "json",
...
<template>
  <div class="wrapper">
    ...
    <BModal v-model="modal" title="Form <?php echo e($moduleTitle); ?>"  size="md" no-footer :no-close-on-backdrop="true">
    <div  >
...
<?php /**PATH /Users/haimac/Documents/jobs/side/rental-api/resources/views/crud/Index.blade.php ENDPATH**/ ?>