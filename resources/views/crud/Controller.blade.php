...
public $search ;
    ...
    { 
        ...
        if( $request->{{ $moduleDBKey }} != '') {
           
            {{ $modelClassName }}::updateData( $posts , $request->{{ $moduleDBKey }}  );
            $action = 'update';        
            $message = " Data has been updated ";
        } else { 
            {{ $modelClassName }}::insertData( $posts );
            $action = 'insert';        
            $message = " Data has been inserted ";
        }
        // Insert Log History
        {{ $modelClassName }}::auditLogs( $request->user()->id , '{{$moduleName}}', $action, $posts);
        return response()->json([
            'status'      => 1 , 
            'posts'         => $posts,
            'message'     => $message
