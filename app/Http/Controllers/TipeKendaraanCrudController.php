...
use App\Models\TipeKendaraanCrud;
...
class TipeKendaraanCrudController extends Controller
{
...
public $search ;
    ...
    { 
        ...
        $this->config =  $configService->prepareSystem('TipeKendaraanCrud' , new TipeKendaraanCrud());
    ...
    { 
        ...
        if( $request->id != '') {
            ...
            TipeKendaraanCrud::updateData( $posts , $request->id  );
            $action = 'update';        
            $message = " Data has been updated ";
        } else { 
            TipeKendaraanCrud::insertData( $posts );
            $action = 'insert';        
            $message = " Data has been inserted ";
        }
        // Insert Log History
        TipeKendaraanCrud::auditLogs( $request->user()->id , 'TipeKendaraanCrud', $action, $posts);
        return response()->json([
            'status'      => 1 , 
            'posts'         => $posts,
            'message'     => $message
    ...
    public function destroy($ids)
    {
        ...
        try {
            TipeKendaraanCrud::deleteData( $ids );
